...
function create_jwt(payload::Dict{String, Any})::String
    ...
    signature = hmac_sha256(ENV["JWT_SECRET"], "$header_encoded.$payload_encoded")
    signature_encoded = base64urlencode(signature)
    return "$header_encoded.$payload_encoded.$signature_encoded"
end

function verify_jwt(token::AbstractString)::Tuple{Bool, Any}
    parts = split(token, ".")
    if length(parts) != 3
    ...
    expected_signature = hmac_sha256(ENV["JWT_SECRET"], "$header_encoded.$payload_encoded")
